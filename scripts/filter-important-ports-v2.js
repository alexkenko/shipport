...
async function filterImportantPorts() {
  ...
  const importantCountries = [
    ...
    'DZ', 'LY', 'IL', 'LB', 'JO', 'SY', 'IQ', 'IR', 'OM', 'YE', 'SO', 'DJ', 'ET', 'SD', 'ER', 'SS'
  ];
  ...
  for (let i = 0; i < cleanBatchFiles.length; i++) {
    ...
    try {
      ...
      // Filter ports by country code
      const filteredPorts = portEntries.filter(entry => {
        // Extract country code from the entry - look for pattern like '"CN"', '"US"', etc.
        const countryMatch = entry.match(/"([A-Z]{2})"/);
        if (!countryMatch) return false;
        
        const countryCode = countryMatch[1];
        return importantCountries.includes(countryCode);
      });
      ...
      // Stop when we have enough ports
      if (totalFiltered >= 15000) {
        console.log(`🎯 Reached target of 15,000 ports, stopping...`);
        break;
      }
  ...
  if (importantPorts.length > 0) {
    ...
    sqlContent += `-- Total ports: ${importantPorts.length}\n\n`;
