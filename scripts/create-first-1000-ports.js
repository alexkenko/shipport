...
function createFirst1000Ports() {
  try {
    console.log('🚢 Reading global ports JSON file...');
    const portsData = JSON.parse(fs.readFileSync('Listofports/global_ports.json', 'utf8'));
    
    // Take first 1000 ports
    const first1000 = portsData.slice(0, 1000);
    console.log(`📊 Processing first ${first1000.length} ports...`);
    
    let sql = '-- Import first 1000 ports\n';
    ...
    const portRows = first1000.map((port, index) => {
      const portId = `${index + 1}`.padStart(6, '0');
    ...
    // Write to file
    fs.writeFileSync('sql/import_first_1000_ports.sql', sql);
    console.log('✅ SQL file created: sql/import_first_1000_ports.sql');
    console.log(`📊 Total ports processed: ${first1000.length}`);
