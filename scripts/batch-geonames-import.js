...
const BATCH_SIZE = 1000; // Process in batches of 1000 ports
...
// Process GeoNames data using streaming and write in batches
async function processGeoNamesDataBatched() {
  console.log('🔄 Processing allCountries for ports and harbors (batched)...');
  ...
  const sqlFile = fs.createWriteStream('sql/batched_geonames_ports.sql');
  sqlFile.write(`-- Comprehensive Global Ports and Harbors Import
...
`);
    ...
    if (line.trim()) {
      ...
      if (fields.length >= 19) {
        ...
        if (isPort || isPortFeature) {
          ...
          currentBatch.push(port);
          portCount++;
          
          // Write batch when it reaches BATCH_SIZE
          if (currentBatch.length >= BATCH_SIZE) {
            await writeBatch(sqlFile, currentBatch, batchCount === 0);
            currentBatch = [];
            batchCount++;
            console.log(`📊 Processed ${portCount} ports in ${batchCount} batches...`);
          }
        }
      }
    }
  ...
  console.log(`📊 Processed ${processedCount} locations, found ${portCount} ports and harbors in ${batchCount} batches`);
  return portCount;
}

// Write a batch of ports to SQL file
async function writeBatch(sqlFile, ports, isFirstBatch) {
  return new Promise((resolve) => {
    if (isFirstBatch) {
      sqlFile.write(`INSERT INTO ports (
...
async function main() {
  try {
    console.log('🚢 Starting COMPREHENSIVE port import from GeoNames.org (batched)...');
    
    // Download the most comprehensive dataset
    await downloadGeoNamesData();
    await extractZip();
    
    // Process the data using batching
    const portCount = await processGeoNamesDataBatched();
    ...
    console.log(`📄 SQL file created: sql/batched_geonames_ports.sql`);
    console.log(`📊 Total ports prepared for import: ${portCount}`);
    console.log('✅ Ready for import! This should give you tens of thousands of ports worldwide.');
...
main()
  .then(() => {
    console.log('✅ Batched GeoNames port import preparation completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ Batched GeoNames port import preparation failed:', error);
    process.exit(1);
  });
