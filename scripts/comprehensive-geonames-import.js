...
// GeoNames has multiple files we can use for comprehensive port data
const GEONAMES_FILES = [
  {
    url: 'https://download.geonames.org/export/dump/allCountries.zip',
    name: 'allCountries',
    description: 'All countries (includes all features)'
  },
  {
    url: 'https://download.geonames.org/export/dump/cities500.zip',
    name: 'cities500',
    description: 'Cities with population > 500'
  },
  {
    url: 'https://download.geonames.org/export/dump/cities1000.zip',
    name: 'cities1000',
    description: 'Cities with population > 1000'
  }
];
...
async function downloadGeoNamesData(url, filename) {
  return new Promise((resolve, reject) => {
    console.log(`🌍 Downloading ${filename}...`);
    
    const file = fs.createWriteStream(`temp/${filename}.zip`);
...
async function extractZip(filename) {
  return new Promise((resolve, reject) => {
    ...
    const zipFile = new zip({ file: `temp/${filename}.zip` });
    
    zipFile.on('ready', () => {
      zipFile.extract(`${filename}.txt`, 'temp/', (err) => {
        if (err) {
...
// Process GeoNames data to extract ALL ports and harbors
function processGeoNamesData(filename) {
  console.log(`🔄 Processing ${filename} for ports and harbors...`);
  
  const data = fs.readFileSync(`temp/${filename}.txt`, 'utf8');
  ...
  const portKeywords = [
    ...
    'porto', 'puerto', 'port', 'harbor', 'harbour', 'marina', 'dock', 'wharf',
    'pier', 'terminal', 'seaport', 'havn', 'hamn', 'satama', 'liman', 'limani',
    'luka', 'pristan', 'pristanishche', 'bandom', 'bandar', 'pelabuhan',
    'porto', 'puerto', 'port', 'harbor', 'harbour', 'marina', 'dock', 'wharf',
    'pier', 'terminal', 'seaport', 'havn', 'hamn', 'satama', 'liman', 'limani',
    'luka', 'pristan', 'pristanishche', 'bandom', 'bandar', 'pelabuhan',
    'porto', 'puerto', 'port', 'harbor', 'harbour', 'marina', 'dock', 'wharf',
...
// Country code to name mapping (expanded)
function getCountryName(countryCode) {
  const countryMap = {
    ...
    'SY': 'Syria', 'IQ': 'Iraq', 'IR': 'Iran', 'AF': 'Afghanistan', 'PK': 'Pakistan',
    'BD': 'Bangladesh', 'LK': 'Sri Lanka', 'MV': 'Maldives', 'BT': 'Bhutan',
    'NP': 'Nepal', 'MM': 'Myanmar', 'LA': 'Laos', 'KH': 'Cambodia', 'BN': 'Brunei',
    'TL': 'East Timor', 'FJ': 'Fiji', 'PG': 'Papua New Guinea', 'SB': 'Solomon Islands',
    'VU': 'Vanuatu', 'NC': 'New Caledonia', 'PF': 'French Polynesia', 'WS': 'Samoa',
    'TO': 'Tonga', 'KI': 'Kiribati', 'TV': 'Tuvalu', 'NR': 'Nauru', 'PW': 'Palau',
    'FM': 'Micronesia', 'MH': 'Marshall Islands', 'AS': 'American Samoa',
    'GU': 'Guam', 'MP': 'Northern Mariana Islands', 'VI': 'U.S. Virgin Islands',
    'PR': 'Puerto Rico', 'DO': 'Dominican Republic', 'HT': 'Haiti', 'CU': 'Cuba',
    'JM': 'Jamaica', 'BS': 'Bahamas', 'BB': 'Barbados', 'TT': 'Trinidad and Tobago',
    ...
    'GS': 'South Georgia and the South Sandwich Islands', 'BQ': 'Bonaire, Sint Eustatius and Saba',
    'CW': 'Curaçao', 'SX': 'Sint Maarten', 'AW': 'Aruba', 'TC': 'Turks and Caicos Islands',
    'BM': 'Bermuda', 'GL': 'Greenland', 'FO': 'Faroe Islands', 'SJ': 'Svalbard and Jan Mayen',
...
-- Generated from GeoNames.org data (ALL countries)
...
async function main() {
  try {
    ...
    const dataset = GEONAMES_FILES[0]; // allCountries
    await downloadGeoNamesData(dataset.url, dataset.name);
    await extractZip(dataset.name);
    
    // Process the data
    const ports = processGeoNamesData(dataset.name);
    ...
    // Write SQL file
    fs.writeFileSync('sql/comprehensive_geonames_ports.sql', sql);
    console.log(`📄 SQL file created: sql/comprehensive_geonames_ports.sql`);
...
main()
  .then(() => {
    console.log('✅ Comprehensive GeoNames port import preparation completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ Comprehensive GeoNames port import preparation failed:', error);
    process.exit(1);
  });
