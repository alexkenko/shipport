...
async function importRealPorts() {
  try {
    console.log('🚢 Starting import of REAL ports from geonames_ports_import.sql...');
...
importRealPorts()
  .then(() => {
    console.log('\n✅ Real ports import completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n❌ Real ports import failed:', error);
    process.exit(1);
  });
