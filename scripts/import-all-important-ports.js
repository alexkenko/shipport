...
// Script to import all important ports efficiently
async function importAllImportantPorts() {
  console.log('🚢 Importing all important maritime ports...');
  
  const batchesDir = 'sql';
  const cleanBatchFiles = fs.readdirSync(batchesDir)
    .filter(file => file.startsWith('clean_important_batch_') && file.endsWith('.sql'))
    .sort();
