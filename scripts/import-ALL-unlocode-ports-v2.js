...
async function importALLUNLOCODEPorts() {
  try {
    ...
    for (const filePath of csvFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (totalProcessed % 5000 === 0) {
          console.log(`📊 Processed ${totalProcessed} locations, found ${allPorts.length} ports so far...`);
        }
      }
    }
    ...
    console.log(`🚢 Ports found: ${allPorts.length}`);
...
function generateSQL(ports) {
  let sql = '-- Import ALL UN/LOCODE locations\n';
