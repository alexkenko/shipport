...
async function importPortsInChunks() {
  try {
    ...
    // Process ports in chunks of 100 for easier handling
    const chunkSize = 100;
    const totalChunks = Math.ceil(portsData.length / chunkSize);
    
    console.log(`📦 Processing ${totalChunks} chunks of ${chunkSize} ports each...`);
    
    let sql = '-- Import 3000+ ports in chunks\n';
    sql += 'DELETE FROM ports;\n\n';
    
    for (let i = 0; i < totalChunks; i++) {
      const start = i * chunkSize;
      const end = Math.min(start + chunkSize, portsData.length);
      const chunk = portsData.slice(start, end);
      
      console.log(`📥 Processing chunk ${i + 1}/${totalChunks} (ports ${start + 1}-${end})...`);
      
      // Transform ports to match our schema
      const transformedPorts = chunk.map((port, index) => {
        const portId = `${i * chunkSize + index + 1}`.padStart(6, '0');
      ...
      const chunkSQL = transformedPorts.map(port => {
        return `('${port.world_port_index}', '${port.region_name}', '${port.main_port_name}', '${port.alternate_port_name}', '${port.un_locode}', '${port.country_code}', '${port.country_name}', '${port.world_water_body}', '${port.iho_sea_area}', ${port.latitude}, ${port.longitude}, '${port.harbor_size}', '${port.harbor_type}', '${port.harbor_use}', '${port.shelter_afforded}', ${port.tidal_range}, ${port.entrance_width}, ${port.channel_depth}, ${port.anchorage_depth}, ${port.cargo_pier_depth}, ${port.max_vessel_length}, ${port.max_vessel_beam}, ${port.max_vessel_draft}, ${port.pilotage_compulsory}, ${port.pilotage_available}, ${port.tugs_assistance}, ${port.facilities_wharves}, ${port.facilities_anchorage}, ${port.facilities_container}, ${port.facilities_oil_terminal}, ${port.facilities_lng_terminal}, '${port.search_text}', NOW(), NOW())`;
      }).join(',\n');
    ...
    // Write to file
    fs.writeFileSync('sql/import_3000_ports_chunked.sql', sql);
    console.log('✅ Chunked SQL file created: sql/import_3000_ports_chunked.sql');
...
function getCountryCode(country) {
  const countryCodes = {
    'United States': 'US', 'China': 'CN', 'Singapore': 'SG', 'Netherlands': 'NL', 'Germany': 'DE',
    'United Kingdom': 'GB', 'Japan': 'JP', 'South Korea': 'KR', 'France': 'FR', 'Spain': 'ES',
    'Italy': 'IT', 'Belgium': 'BE', 'Canada': 'CA', 'Australia': 'AU', 'Brazil': 'BR',
    'India': 'IN', 'Russia': 'RU', 'Turkey': 'TR', 'Greece': 'GR', 'Poland': 'PL',
    'Sweden': 'SE', 'Norway': 'NO', 'Denmark': 'DK', 'Finland': 'FI', 'Portugal': 'PT',
    'Ireland': 'IE', 'Mexico': 'MX', 'South Africa': 'ZA', 'Egypt': 'EG', 'Thailand': 'TH',
    'Vietnam': 'VN', 'Philippines': 'PH', 'Malaysia': 'MY', 'Indonesia': 'ID', 'Argentina': 'AR',
    'Chile': 'CL', 'Colombia': 'CO', 'Panama': 'PA', 'Morocco': 'MA', 'Saudi Arabia': 'SA',
    'United Arab Emirates': 'AE', 'Oman': 'OM', 'Israel': 'IL', 'Taiwan': 'TW', 'New Zealand': 'NZ'
  };
...
function getRegionName(continent) {
  const regions = {
    'Asia': 'East Asia', 'Europe': 'Western Europe', 'North America': 'North America',
