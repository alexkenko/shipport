...
async function importGeonamesPorts() {
  try {
    console.log('🚢 Starting import of GeoNames ports (PORTS TABLE ONLY)...');
  ...
  created_at, updated_at
      ...
      try {
        ...
        if (error) {
          ...
          await importBatchAsData(batchValues, batchNumber);
        } else {
...
// Alternative method: parse batch and insert as data
async function importBatchAsData(batchValues, batchNumber) {
  try {
    console.log(`🔄 Trying alternative import method for batch ${batchNumber}...`);
    ...
    console.error(`❌ Error with alternative import for batch ${batchNumber}:`, err);
  }
}
...
importGeonamesPorts()
  .then(() => {
    console.log('\n✅ GeoNames port import completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n❌ GeoNames port import failed:', error);
    process.exit(1);
  });
